...
class Model:
    ...
    def compute_w6(self):
        importance = []
        for v in range(len(self.var_list)):
            im = tf.gradients(tf.nn.l2_loss(self.probs6), self.var_list[v])
    ...
    def compute_w7(self):
        importance = []
        for v in range(len(self.var_list)):
            im = tf.gradients(tf.nn.l2_loss(self.probs7), self.var_list[v])
    ...
    def compute_w8(self):
        importance = []
        for v in range(len(self.var_list)):
            im = tf.gradients(tf.nn.l2_loss(self.probs8), self.var_list[v])
    ...
    def compute_w9(self):
        importance = []
        for v in range(len(self.var_list)):
            im = tf.gradients(tf.nn.l2_loss(self.probs9), self.var_list[v])
    ...
    def compute_w10(self):
        importance = []
        for v in range(len(self.var_list)):
            im = tf.gradients(tf.nn.l2_loss(self.probs10), self.var_list[v])
    ...
    def update_MAS_loss6(self, lam):
        if not hasattr(self, "MAS_loss6"):
            self.MAS_loss6 = self.cross_entropy6
        for v in range(len(self.var_list)):
            self.MAS_loss6 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step6 = tf.train.GradientDescentOptimizer(0.001).minimize(self.MAS_loss6)
    def update_MAS_loss7(self, lam):
        if not hasattr(self, "MAS_loss7"):
            self.MAS_loss7 = self.cross_entropy7
        for v in range(len(self.var_list)):
            self.MAS_loss7 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step7 = tf.train.GradientDescentOptimizer(0.001).minimize(self.MAS_loss7)
    def update_MAS_loss8(self, lam):
        if not hasattr(self, "MAS_loss8"):
            self.MAS_loss8 = self.cross_entropy8
        for v in range(len(self.var_list)):
            self.MAS_loss8 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step8 = tf.train.GradientDescentOptimizer(0.001).minimize(self.MAS_loss8)
    def update_MAS_loss9(self, lam):
        if not hasattr(self, "MAS_loss9"):
            self.MAS_loss9 = self.cross_entropy9
        for v in range(len(self.var_list)):
            self.MAS_loss9 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step9 = tf.train.GradientDescentOptimizer(0.001).minimize(self.MAS_loss9)
    def update_MAS_loss10(self, lam):
        if not hasattr(self, "MAS_loss10"):
            self.MAS_loss10 = self.cross_entropy10
        for v in range(len(self.var_list)):
            self.MAS_loss10 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step10 = tf.train.GradientDescentOptimizer(0.001).minimize(self.MAS_loss10)
    ...
    def compute_omega1(self, sess, imgset, batch_size,keep_prob):
        ...
        for iter in range(batch_num):
            ...
            Omega_M.append(sess.run(self.omega1, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega2(self, sess, imgset, batch_size,keep_prob):
        ...
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega2, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega3(self, sess, imgset, batch_size,keep_prob):
        ...
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega3, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega4(self, sess, imgset, batch_size,keep_prob):
        ...
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega4, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega5(self, sess, imgset, batch_size,keep_prob):
        ...
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega5, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega6(self, sess, imgset, batch_size,keep_prob):
        Omega_M = []
        batch_num = ceil(len(imgset.images) / batch_size)
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega6, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega7(self, sess, imgset, batch_size,keep_prob):
        Omega_M = []
        batch_num = ceil(len(imgset.images) / batch_size)
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega7, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega8(self, sess, imgset, batch_size,keep_prob):
        Omega_M = []
        batch_num = ceil(len(imgset.images) / batch_size)
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega8, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega9(self, sess, imgset, batch_size,keep_prob):
        Omega_M = []
        batch_num = ceil(len(imgset.images) / batch_size)
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega9, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
    def compute_omega10(self, sess, imgset, batch_size,keep_prob):
        Omega_M = []
        batch_num = ceil(len(imgset.images) / batch_size)
        for iter in range(batch_num):
            index = iter * batch_size
            Omega_M.append(sess.run(self.omega10, feed_dict={self.x: imgset.images[index:(index + batch_size - 1)],self.keep_prob:keep_prob}))
        self.Omega_M = np.sum(Omega_M, 0) / batch_num
        #print("参数重要性：",self.Omega_M)
        return self.Omega_M
